...
// Handle a vote on a poll with cooldown mechanism
async function handleVote(
  ...
  // Check if user has already voted and has a completed transaction
  ...
  try {
    ...
    const vote_result = await vote(
      ...
      Number(optionIndex),
      signerPriv,
      await getRing(Number(userId), signerPubKey)
    );

    if (!vote_result || !vote_result.transaction_hash) {
    ...
    await new Promise<void>((resolve, reject) => {
      db.run(
        ...
        [pollId, (option as any).id, userId, tx_hash],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
...
bot.on('callback_query', async (ctx) => {
  ...
  try {
    ...
    if (isVoting) {
      ...
      if (votingStatus === 'pending' && inCooldown) {
        return ctx.answerCbQuery('⏳ Your previous vote is still being processed. Please wait before trying again.');
      } else if (votingStatus === 'failed' && inRetryCooldown) {
        return ctx.answerCbQuery('⚠️ Your previous vote attempt failed. Please try again in a few minutes.');
      }
    }
      ...
      .then(async (voteResult) => {
        if (voteResult.success) {
          ...
          if (voteResult.tx_hash) {
            // DM the user to confirm the vote
          ...
          await ctx.telegram.sendMessage(
            userId, // The ID of the user who voted
            `❌ Failed to record your vote: ${voteResult.message}`,
            { parse_mode: 'HTML' }
          );
